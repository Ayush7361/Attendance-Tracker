...
function AttendanceCalendar({ schedule, onDaySaved }) {
  ...
  // Load month records whenever the visible month changes
  useEffect(() => {
    loadMonthRecords();
  }, [visibleMonth]);

  // When schedule changes, refresh the currently selected day
  useEffect(() => {
    if (selectedDate) {
  ...
  async function handleDayClick(date) {
    ...
    try {
      ...
      if (res.data && Array.isArray(res.data.subjects) && res.data.subjects.length > 0) {
        // Existing saved record → restore exact choices
        setSubjectsList(
        ...
        // Brand new day → force user to choose
  ...
  function setSubjectsFromSchedule(date, fallbackTotal, fallbackAttended) {
    ...
    if (Array.isArray(scheduledSubjects) && scheduledSubjects.length > 0) {
      // New day from schedule → attended starts as null (undecided)
      setSubjectsList(
        scheduledSubjects.map((subName) => ({
          name: subName,
          attended: null,
        }))
      );
      ...
      for (let i = 1; i <= scheduledSubjects; i++) {
        list.push({ name: `Class ${i}`, attended: null });
      }
  ...
  function handleAddCustomSubject(e) {
    ...
    if (!trimmed) return;
    // Custom subjects default to attended = true (user can still change it)
  ...
  const attendedCount = subjectsList.filter((s) => s.attended === true).length;
  const allDecided = subjectsList.every(
    (s) => s.attended === true || s.attended === false
  );

  async function handleSave() {
    if (!selectedDate || !allDecided || subjectsList.length === 0) return;
  ...
  const isTodaySelected =
    selectedDate && formatDateKey(selectedDate) === formatDateKey(new Date());
      ...
      <div className="calendar-body">
        ...
        {selectedDate && (
          <div className="day-checklist-panel">
            ...
            {loadingDay ? (
              <p className="loading-day">Loading...</p>
            ) : (
              <>
                    ...
                    <div key={idx} className="subject-row">
                      ...
                      <div className="row-controls">
                        <div className="attendance-toggle-group">
                          <button
                            type="button"
                            className={`toggle-option attended ${
                              sub.attended === true ? "active" : ""
                            }`}
                          ...
                          <button
                            type="button"
                            className={`toggle-option missed ${
                              sub.attended === false ? "active" : ""
                            }`}
                ...
                <form
                  ...
                  className="add-extra-subject-form"
                >
                  <input
                    type="text"
                ...
                <div className="checklist-stats">
                  ...
                  <span>
                    Attended: <strong>{attendedCount}</strong>
                  </span>
                </div>

                {!allDecided && subjectsList.length > 0 && (
                  <p className="hint">
                    Please mark Attended or Not Attended for every class before
                    saving.
                  </p>
                )}

                <button
                  className="save-day-record-btn"
                  onClick={handleSave}
                  disabled={
                    saving || !allDecided || subjectsList.length === 0
                  }
                >
